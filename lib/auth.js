...
function addSignature(key, secret, uri, body) {
    ...
    if (body) {
        ...
        console.log("api key before encoding: ", key);
        console.log("body before encoding: ", body);
        console.log("signatureBase before adding body: ", signatureBase);
        signatureBase += encodeURIComponent(body);
        console.log("signatureBase + body => ", signatureBase);
        console.log("uri for POST: ", uri);
    }

    // creating a hash
    var hash = SHA256(signatureBase, secret).toString(hex);

    //console.log("Hashed signatureBase: ", signatureBase);
